...
def pytorch_attack_against_baard(data_name, model_name, att, epsilons, idx, baard_param, fresh_att=False, fresh_def=True):
    ...
    print('Start training {} model on {}...'.format(model_name, data_name))
    ...
    print('Accuracy on {} filtered test set: {:.2f}%'.format(y_test.shape[0], acc_perfect * 100))
    ...
    for e in epsilons:
        try:
            ...
            if os.path.exists(path_adv) and not fresh_att:
                ...
                start = time.time()
                adv = get_advx_untargeted(model, data_name, att, eps=e, device=device, X=X_att, y=None, batch_size=BATCH_SIZE)
