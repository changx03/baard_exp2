...
def net_generation_and_train(X_train, y_train, net_type):
    ...
    net = net_type(n_features=n_features, n_classes=n_classes)
    optimizer = optim.SGD(net.parameters(),
                          lr=0.1, momentum=0.9)

    clf = PyTorchClassifier(model=net, loss=loss, input_shape=(1,n_features,),
                                nb_classes=n_classes, optimizer=optimizer)
...
def plot_detector_decision_region(X, clf, detector):
    ...
    colors = ['blue', 'red', 'green', 'gray', 'green', 'cyan']
    cmap = colors[:4]
...
# train a detector
# create a dataset that contains the clean sample (with label 0) + the
...
attack_class = 'apgd' # can be apgd or apgd_detector
