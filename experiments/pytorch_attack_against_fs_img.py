...
def pytorch_attack_against_fs_img(data_name, model_name, att, epsilons, idx):
    ...
    path_fs = os.path.join(path_results, 'data', '{}_fs.pt'.format(data_name))
    if os.path.exists(path_fs):
        print('[Defence] Found existing a pretrained FS model:', path_fs)
        detector.load(path_fs)
    else:
        detector.fit(X_train, y_train, epochs=EPOCHS, verbose=1)
        detector.save(path_fs)
        print('[Defence] Saved fs to:', path_fs)
...
if __name__ == '__main__':
    ...
    print('seed:', seed)
    pytorch_attack_against_fs_img(data, model_name, att, epsilons, idx)
...
# python3 ./experiments/pytorch_attack_against_fs_img.py -d mnist -i 0 -a apgd -e 0.063 0.3 1.0
